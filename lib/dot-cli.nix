...
pkgs.writeShellApplication {
  ...
  text = ''
        ...
        update_helium_extensions() {
          local browsers_file="$PROJECT_DIR/modules/core/packages/desktop/browsers.nix"

          if [[ ! -f "$browsers_file" ]]; then
            print_warn "Helium browser module not found, skipping extension updates"
            return 0
          fi

          print_info "Updating Helium extension CRX hashes..."
          python3 - "$browsers_file" <<'PY'
    import base64
    import hashlib
    import json
    import re
    ...
    browser_file = Path(sys.argv[1])
    text = browser_file.read_text()

    entry_re = re.compile(
        r'(?P<id>[a-p]{32}) = \{\n'
        r'\s+name = "(?P<name>[^"]+)";\n'
        r'\s+version = "(?P<version>[^"]+)";\n'
        r'\s+crx = pkgs\.fetchurl \{\n'
        r'\s+name = "(?P=id)\.crx";\n'
        r'\s+url = crxUrl "(?P=id)";\n'
        r'\s+hash = "(?P<hash>sha256-[^"]+)";\n'
        r'\s+\};\n'
        r'\s+\};'
    )
    ...
    if not entries:
        raise SystemExit("no Helium extension entries found")

    context = ssl.create_default_context()

    def crx_url(extension_id: str) -> str:
        x_param = urllib.parse.quote(f"id={extension_id}&installsource=ondemand&uc", safe="")
        return (
    ...
    def fetch_crx(extension_id: str) -> bytes:
        request = urllib.request.Request(
            crx_url(extension_id),
            headers={"User-Agent": "dotfiles-helium-extension-updater/1.0"},
        )
    ...
    def crx_zip_offset(data: bytes) -> int:
        ...
        if offset < 0:
            raise ValueError("zip payload not found in CRX")
        return offset

    def extension_version(data: bytes) -> str:
        offset = crx_zip_offset(data)
        manifest_bytes = data[offset:]
        with zipfile.ZipFile(BytesIO(manifest_bytes)) as archive:
    ...
    for entry in entries:
        extension_id = entry.group("id")
        name = entry.group("name")
        old_version = entry.group("version")
        old_hash = entry.group("hash")

        data = fetch_crx(extension_id)
        new_version = extension_version(data)
        new_hash = sri_hash(data)

        if old_version != new_version or old_hash != new_hash:
            changed = True
        ...
        block = entry.group(0)
        block = re.sub(r'version = "[^"]+";', f'version = "{new_version}";', block, count=1)
        block = re.sub(r'hash = "sha256-[^"]+";', f'hash = "{new_hash}";', block, count=1)
        replacements[entry.span()] = block
    ...
    if new_text != text:
        browser_file.write_text(new_text)

    if not changed:
        print("Helium extensions already match Chrome Web Store metadata")
    PY
        }
