...
let
  git-ai-commit = pkgs.writeShellScriptBin "git-ai-commit" ''
        ...
        prompt=$(cat <<'EOF'
    Generate a git commit message for ONLY the staged changes described in the provided git context.
    ...
    - Do not inspect files or run commands; the provided context is enough.
    EOF
        )
